...
public class AlloyFurnaceBlockEntity extends LootableContainerBlockEntity
    ...
    public Item getRecipe(Item ingredient1, Item ingredient2) {
        if (ingredient1 == Items.AIR || ingredient2 == Items.AIR) {
            return null;
        }
        Vector<Item> itemPair = new Vector<Item>(Arrays.asList(ingredient1, ingredient2));
        return AlloyFurnaceBlockEntity.createRecipes().get(itemPair);
    }
    ...
    private static boolean canAcceptRecipeOutput(AlloyFurnaceBlockEntity blockEntity){
        if (blockEntity.inventory.get(3).getCount() < blockEntity.inventory.get(3).getMaxCount()){
            ...
            return blockEntity.getStack(3).getItem() == blockEntity.getRecipe(blockEntity.getStack(0).getItem(), blockEntity.getStack(1).getItem());
        } else {
            return false;
        }
    }
            ...
            if(canAcceptRecipeOutput(blockEntity) && blockEntity.hasValidRecipe()){
                ...
                if (blockEntity.currentProgress == blockEntity.maxProgress){
                    if (blockEntity.inventory.get(3).isEmpty()){
                        blockEntity.setStack(3, new ItemStack(blockEntity.getRecipe(blockEntity.getStack(1).getItem(), blockEntity.getStack(0).getItem())));
                    } else {
